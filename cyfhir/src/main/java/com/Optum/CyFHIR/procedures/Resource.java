...
public class Resource {
    ...
    public void attachLooseReferences(ArrayList<String> fullUrls, Transaction tx) {
        String prefix = "MATCH (a) MATCH (b:entry) WHERE NOT (a)-[:reference]->() AND b.fullUrl IN [\"";
        String suffix = "\"] AND a.reference = b.fullUrl CREATE (a)-[:reference]->(b)";
    ...
    json.forEach((key, o) -> {
        if (o instanceof ArrayList) {
            ...
            if (a.size() > 0 && a.get(0) instanceof LinkedHashMap) {
                a.forEach((t) -> {
                    Map<String, Object> map = (Map<String, Object>) t;
                    ...
                    if (map.containsKey("reference")) {
                        FhirRelationship ref = new FhirRelationship();
                        ref.setParentNode(n);
                        ref.setChildRelationship(map.get("reference").toString(), key);
                        relationships.add(ref);
                    } else {
                        ArrayList<FhirRelationship> relations = getRelationshipsFromMap(tx, map, key, n, true,
                                resourceId);
