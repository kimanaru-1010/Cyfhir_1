...
Java 17: Download ZIP Eclipse Temurin JDK 17, extract
Maven 3.8.6: Download từ Apache archive, extract
Set JAVA_HOME, M2_HOME, thêm vào PATH
Bước 2: Build plugin CyFHIR

bash bin/build-cyfhir.sh
→ Build Java project → tạo CyFHIR.jar 51MB → copy vào cyfhir/plugins/

Bước 3: Fix Dockerfile
Đổi node:14.5.0-alpine → node:18-alpine
Xóa apk add python3 make g++ (SSL cert lỗi)
Bước 4: Chạy Docker

cd docker-compose && docker-compose up -d
→ 2 container: Neo4j + Express-backend

Bước 5: Thêm endpoint load từ FHIR server
Tạo 3 file sửa:

neo4jController.ts: Thêm loadFromFhirServer() + loadAllResources() + sanitizeEntry() (strip binary data)
router.ts: Thêm route /LoadFromFhirServer + /LoadAllResources
cypherController.ts: Fix escape JSON (\\, ", ')
Bước 6: Fix 3 bug chính
JSON escape: Binary có HTML với \" trong data field → escape thiếu → thêm backslash escape
URL rewrite: FHIR server trả về next link là http://hapi-fhir:8080/fhir (tên internal Docker) → rewrite thành http://172.16.12.230:8084/fhir
